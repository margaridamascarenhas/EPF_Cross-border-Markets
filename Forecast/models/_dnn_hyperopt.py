...
set_random_seeds(42) #Always 42 except for BE_base CW 730, 112, 56 , then is 25
...
def _hyperopt_objective(hyperparameters, trials, trials_file_path, max_evals, nlayers, dfTrain, dfTest, 
    ...
    #dfTrain_cw = dfTrain.loc[dfTrain.index[-1] - pd.Timedelta(weeks=52) * calibration_window + pd.Timedelta(hours=1):]
    dfTrain_cw = dfTrain.loc[dfTrain.index[-1] - pd.Timedelta(days=calibration_window) + pd.Timedelta(hours=1):]
...
def hyperparameter_optimizer(path_datasets_folder=os.path.join('.', 'datasets'), 
                             ...
                             experiment_id=None, begin_test_date=None, end_test_date=None, Select_Inputs='All'):
    ...
    trials_file_name = 'DNN_hyperparameters_nl' + str(nlayers) + '_dat' + str(dataset) + \
                       ...
                       '_' + str(experiment_id) + str(Select_Inputs)
    ...
    dfTrain, dfTest = read_data(dataset=dataset, years_test=years_test, path=path_datasets_folder,
                                begin_test_date=begin_test_date, end_test_date=end_test_date, Select_Inputs=Select_Inputs)
