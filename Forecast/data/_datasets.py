...
def read_data(path, dataset='PJM', years_test=2, begin_test_date=None, end_test_date=None, Select_Inputs='All'):
    ...
    if dataset=='BE_Data_UTC' or dataset=='Add_BE_Data_UTC':
        cet_timezone = pytz.timezone('Europe/Brussels')
    if dataset=='SE3_Data_UTC' or dataset=='Add_SE3_Data_UTC':
        cet_timezone = pytz.timezone('Europe/Stockholm')

    data_aux.index = pd.to_datetime(data_aux.index, dayfirst=False)
    data_aux.index = data_aux.index.tz_localize(pytz.utc)
    data_aux.index = data_aux.index.tz_convert(cet_timezone)
    data_aux.index = data_aux.index.tz_localize(None)
    data_aux = data_aux[~data_aux.index.duplicated(keep='first')]
    data_aux = data_aux.resample('h').interpolate()
    
    selected_columns = ['Price'] + Select_Inputs
    data = data_aux[selected_columns].copy()
